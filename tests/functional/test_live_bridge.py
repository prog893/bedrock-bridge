...
real Bedrock. These assert on the response envelope (success shape or mapped
recovery error), not on model prose, so they are deterministic despite calling
a real model.
...
def test_text_turn_succeeds(bridge):
...
def test_empty_assistant_turn_does_not_500(bridge):
    status, body = bridge.messages({
        "max_tokens": 64,
        "messages": [
...
# sanitized by the bridge, not rejected by Bedrock. We send a tool definition
# plus a forcing prompt; success is "not a 400 charset ValidationException".
def test_illegal_tool_name_is_sanitized(bridge):
    status, body = bridge.messages({
        "max_tokens": 128,
        "tools": [{
...
# turn succeeds.
def test_image_to_text_only_model_is_stripped(bridge):
    status, body = bridge.messages({
        "max_tokens": 64,
        "messages": [{"role": "user", "content": [
            {"type": "text", "text": "Describe what you can see, if anything."},
            {"type": "image", "source": {
                "type": "base64", "media_type": "image/png", "data": _RED_PIXEL}},
        ]}],
    })
    assert status == 200, body
...
def test_image_in_tool_result_roundtrips_on_image_model(bridge_image):
...
def test_streaming_text_turn(bridge):
...
# for these tests (we assert on envelope shape, not perception).
_RED_PIXEL = (
