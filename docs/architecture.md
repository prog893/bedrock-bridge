...
- **System-role entries in `messages` are folded into user turns.** The Anthropic API accepts `{"role": "system", ...}` inside `messages` for mid-conversation operator instructions, and Claude Code sends its `# Environment` block that way as the last entry. Converse only has `user` and `assistant` roles; Bedrock rejects a system entry mid-history ("This model doesn't support system messages") and, as the final entry, as a non-user last turn ("requires the last turn in the conversation to be a user message"). Each such entry becomes a `<system-reminder>`-tagged text block appended to the user turn before it (or prepended to the next one when there is none), which keeps user/assistant alternation intact. Entries with empty content are dropped.
